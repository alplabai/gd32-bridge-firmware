 ...
 *
 * MCUboot's abstract flash-area interface (every port implements this
 * header + the functions below over its own flash driver). This is the
 * struct/prototype SET MCUboot requires, not the implementation -- see
 * flash_map_backend.c for the stub, and the workflow summary for what a
 * real backend over hal/fmc_ota.c would add.
 */
